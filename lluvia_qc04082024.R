...
# Print the current working directory at the global level
print(paste("Global working directory:", getwd()))
...
sourceref <- read.csv("./datos_estaciones/datos_estaciones_test2.csv",
                      fileEncoding = "latin1")
...
# Se identifican los valores en los que hay más de 3 valores menores que los
# anteriores de forma consecutiva
# https://stackoverflow.com/a/71731518/4268720
# Flag2 asigna TRUE si hay más de 3 valores consecutivos que sean menores
# que los anteriores
...
dsn_pwd <- "011995" # Specify your password. e.g. "xxx"

tryCatch({
  print("Connecting to Database…")
  con <- DBI::dbConnect(RPostgres::Postgres(), dbname = dsn_database, 
...
sql_constraint <- 'ALTER TABLE qctable 
                  ADD CONSTRAINT unique_estacion_fecha 
                  UNIQUE (estacion, fecha);'

tryCatch({
  dbExecute(con, sql_constraint)
}, error = function(e) {
  print("Constraint may already exist")
})

# Insertar datos con ON CONFLICT DO NOTHING
sql_insert <- 'INSERT INTO qctable (estacion, est_id, medicion, medicion_interp, 
               lluvia_inst_corr, error, fecha)
               VALUES ($1, $2, $3, $4, $5, $6, $7)
               ON CONFLICT (estacion, fecha) DO NOTHING;'

# Inicializar contador de registros nuevos
nuevos_registros <- 0

# Reemplazar dbWriteTable con inserción fila por fila usando la restricción
for(i in 1:nrow(lluvia_7_dias_inst)) {
  row <- lluvia_7_dias_inst[i, ]
  result <- dbExecute(con, sql_insert,
            list(row$estacion, row$est_id, row$medicion, row$medicion_interp,
                 row$lluvia_inst_corr, row$error, row$fecha))
  nuevos_registros <- nuevos_registros + result
}

# Mostrar mensaje con el número de registros nuevos
print(paste("Se han agregado", nuevos_registros, "nuevos registros a la tabla"))
